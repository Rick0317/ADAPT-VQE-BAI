from src.pools import SD # Other Options: QE, SingletGSD, CEO, DVG_CEO
from src.molecules import create_lih # create_h2, create_h3, create_h4, create_beh2

from algorithms.adapt_vqe import AdaptVQE # Comment this line to use the original AdaptVQE

if __name__ == '__main__':
    r = 1.5
    molecule = create_lih(r)
    pool = SD(molecule)
    print(f"Pool size: {pool.size}")

    # Memory-optimized parameters
    # Reduced from original values to prevent memory issues
    adapt_vqe = AdaptVQE(pool=pool,
                        molecule=molecule,
                        max_adapt_iter=30,      # Reduced from 100
                        max_opt_iter=30,        # Reduced from 100
                        grad_threshold=1e-4,
                        vrb=True,
                        optimizer_method='l-bfgs-b',
                        shots_assignment='uniform',
                        k=50,                   # Reduced from 100
                        shots_budget=512,       # Reduced from 1024
                        N_experiments=1         # Reduced from 2
                        )

    adapt_vqe.run() 